...
{block name="b2b_modal_base_content_inner_scrollable_inner_actions_inner"}
    <form method="get" class="form--inline" action="{url contactId=$contactId}">
...
{block name="b2b_modal_base_content_inner_scrollable_inner_content_inner"}
    ...
    {elseif !$gridState.data|count}
        {include file="frontend/_includes/messages.tpl" type="info" content="{s name="EmptyAssignmentRoleList"}There are no contingents yet. You can create them in the contingent managmenet panel.{/s}"}
    {/if}
    ...
    {if $gridState.data|count}
        ...
        {foreach $gridState.data as $row}
            <div class="panel--tr row--contingent-selection" data-row-id="{$row->id}">
                <form action="{url action=assign contactId=$contactId}" method="post" class="b2b--assignment-form ignore--b2b-ajax-panel {b2b_acl controller=b2bcontactcontingent action=assign}">
