    ...
    {if $gridState.data|count}
        ...
        {foreach $gridState.data as $row}
            <div class="panel--tr row--budget-selection" data-row-id="{$row->id}">
                <form action="{url controller=$assignmentController action=assign}"
                      method="post"
                      class="b2b--assignment-form ignore--b2b-ajax-panel {b2b_acl controller=$assignmentController action=assign}"
